...
class User extends Dbconfig
{
	...
	public function register()
	{
		...
		if (!empty($_POST["register"]) && $_POST["email"] != '') {
			...
			if ($isUserExist) {
				...
				$insertQuery = "INSERT INTO " . $this->userTable . " (first_name, last_name, email, password, authtoken, mobile) VALUES ('" . $_POST["firstname"] . "','" . $_POST["lastname"] . "','" . $_POST["email"] . "','" . md5($_POST["passwd"]) . "','" . $authtoken . "','" . $mobile . "')";
	...
	{
		// Provide a fallback if adminUserid is not set:
		$adminId = $_SESSION["adminUserid"] ?? 0;
	...
	public function listUser()
	{
		...
		$sqlQuery = "SELECT * FROM " . $this->userTable . " WHERE id != '" . $adminId . "' ";
	...
	{
		$sqlQuery = "SELECT u.id,CONCAT(u.first_name,' ',u.last_name) as fullName,s.admission_no,s.email,s.mobile,s.father_name,s.father_mobile,s.mother_name,s.mother_mobile,s.id as student_id,s.name as studentName,sub.subject as sSubject, s.class as sClass, s.section as sSection,
            (select section from sms_section where section_id=sSection) as sSectionName,clssub.class_id,cls.name as className,sec.section_id,sec.section as sectionName,clsses.session_id,ses.session as sessionName,clssub.subject_id,sub.subject as subjectName,prosub.program_id,pro.program as programName,cls.teacher_id,tea.teacher as teacherName,cls.start_date,cls.end_date,stucls.status
